...
#             image_file = os.path.join(base_dir, 'images', name + '_sat.jpg')
#             gt_file = os.path.join(base_dir, 'gt', name + '_gt.png')
...
#     parser.add_argument('-d', '--base_dir', type=str, default='C:/Program Files (x86)/Study/Degree/Intern/Intern Project/CoANet/data/7b52009b64fd0a2a49e6d8a939753077792b0554',
#         help='Base directory for TIA Dataset.')
#     parser.add_argument('--dataset', type=str, default='7b52009b64fd0a2a49e6d8a939753077792b0554',
#                         choices=['SpaceNet', 'DeepGlobe','7b52009b64fd0a2a49e6d8a939753077792b0554'], help='dataset name')
...
#ZZZ :Cropping image into smaller size due to the reason that original datatset from this code is using satelite image 
...
def create_single_sample_crops(base_dir, crop_type, size, stride):
    dataset = '7b52009b64fd0a2a49e6d8a939753077792b0554'
    crops = os.path.join(base_dir, 'crops')
    
    if not os.path.exists(crops):
        os.mkdir(crops)
        os.mkdir(os.path.join(crops, 'images'))
        os.mkdir(os.path.join(crops, 'gt'))

    name = 'frame0000'  # Name of the sample to process
    image_file = os.path.join(base_dir, 'images', name + '_sat.png')
    gt_file = os.path.join(base_dir, 'gt', name + '_gt.jpg') 

    if not verify_image(image_file):
        print("Image verification failed for:", image_file)
        return
    ...
    if image is None or gt is None:
        print("Image or ground truth loading failed for:", image_file)
        return
    ...
    for x in range(maxx + 1):
        for y in range(maxy + 1):
            im_ = image[x * stride:x * stride + size, y * stride:y * stride + size, :]
            gt_ = gt[x * stride:x * stride + size, y * stride:y * stride + size]
            cv2.imwrite(os.path.join(crops, 'images', '{}_{}_{}.png'.format(name, x, y)), im_)
            cv2.imwrite(os.path.join(crops, 'gt', '{}_{}_{}.jpg'.format(name, x, y)), gt_)

def main():
    base_dir = 'C:/Program Files (x86)/Study/Degree/Intern/Intern Project/CoANet/data/7b52009b64fd0a2a49e6d8a939753077792b0554'  # Update with your dataset directory
    crop_type = 'sample'
    crop_size = 650
    stride = 650

    create_single_sample_crops(base_dir, crop_type, crop_size, stride)
